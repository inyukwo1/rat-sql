...
function(args) _base(output_from=_output_from, data_path=args.data_path) + {
    ...
    local base_bert_enc_size_ = if args.bert_version == "bert-large-uncased-whole-word-masking" then 1024 else 768,
    local base_bert_enc_size = if args.bert_version == "Salesforce/grappa_large_jnt" then 1024 else base_bert_enc_size_,
